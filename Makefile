...
clean:
	...
	rm -f sha3/*.o
	rm -f sha3/*.a
	rm -f algos/argon2ref/*.o

install: clean all
	strip -s stratum-mtp
	sudo -u www-data cp stratum-mtp /var/stratum/
	sudo -u www-data cp run-mtp1.sh /var/stratum/rum-mtp.sh
	sudo -u www-data cp run-mtp2.sh /var/stratum/config/rum-mtp.sh
	sudo -u www-data cp -rf mtp.conf /var/stratum/config/
